...
import {
  ...
  MessageFlags, ModalBuilder, ModalSubmitInteraction, PermissionFlagsBits, StringSelectMenuBuilder,
...
export async function handlePanelButton(interaction: ButtonInteraction): Promise<boolean> {
  ...
  if (id === "面板開啟") {
    ...
    await interaction.reply({ ...mainPanelView(config, Boolean(consent), member), flags: MessageFlags.Ephemeral }); return true;
  }
...
export async function showManagedActivityFromPost(interaction: ButtonInteraction, type: "約會" | "派對", id: string): Promise<void> {
  ...
  if (type === "約會") {
    ...
    await interaction.reply({ flags: MessageFlags.Ephemeral, embeds: [new EmbedBuilder().setColor(0xe879a9).setTitle(`約會｜${post.activity}`).addFields(
  ...
  await interaction.reply({ flags: MessageFlags.Ephemeral, embeds: [new EmbedBuilder().setColor(0x7c5cff).setTitle(`派對｜${party.name}`).addFields(
...
async function finishModeration(interaction: ModalSubmitInteraction, draftId: string) {
  ...
  await prisma.interactionDraft.delete({ where: { id: draft.id } }); await interaction.reply({ content: `✅ 已執行「${moderationActionLabels[action]}」。`, flags: MessageFlags.Ephemeral });
}
