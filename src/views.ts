...
export function dateMessage(post: DatePost, disabled = false) {
  const embed = new EmbedBuilder().setColor(post.status === "OPEN" ? 0xe879a9 : 0x777777)
    ...
    .addFields(
      ...
      { name: "約會 ID", value: `\`${post.id}\``, inline: true }
  ...
  const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
    ...
    new ButtonBuilder().setCustomId(customId("date_cancel", post.id)).setLabel("取消（發起者）").setStyle(ButtonStyle.Danger).setDisabled(disabled)
  );
...
export function partyMessage(party: Party, going: number, waiting: number, disabled = false) {
  ...
  const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
    ...
    new ButtonBuilder().setCustomId(customId("party_cancel", party.id)).setLabel("取消（發起者）").setStyle(ButtonStyle.Danger).setDisabled(disabled)
  );
  return { embeds: [embed], components: [row] };
}
